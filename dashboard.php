...
$cardsStatement = db()->prepare("SELECT card_name, issuer, network, last_four, annual_fee, status FROM cards WHERE user_id = :user_id AND status = 'active' ORDER BY created_at DESC LIMIT 4");
$cardsStatement->execute(['user_id' => $user['id']]);
$cards = $cardsStatement->fetchAll();
$summaryStatement = db()->prepare("SELECT COUNT(*) AS card_count, COALESCE(SUM(annual_fee), 0) AS annual_fees FROM cards WHERE user_id = :user_id AND status = 'active'");
...
<div class="app-shell">
    ...
    <main class="content">
        ...
        <section class="section-heading"><div><p class="eyebrow">YOUR WALLET</p><h2>My cards</h2></div><a class="text-link" href="cards.php">Manage cards →</a></section>
